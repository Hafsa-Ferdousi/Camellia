...
const Recommendations = ({ productId }) => {
  ...
  if (loading) return <div style={{ padding: '20px 0', color: '#555' }}>{t('loading', { defaultValue: 'Loading…' })}</div>;
            ...
            <Link key={product._id} to={`/products/${product._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              ...
              >
                {image ? (
                  ...
                  <div style={{ width: '100%', height: '160px', background: '#f8f5f0', borderRadius: '4px', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', color: '#b8ab98', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{t('noImage')}</div>
                )}
